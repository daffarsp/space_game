...
start_screen_music = pygame.mixer.Sound('rnbforgame.mp3')
game_over_music = pygame.mixer.Sound('illusoryrealm.mp3')
background_music = pygame.mixer.music.load('rnbforgame.mp3')
...
class Enemy(Player):
	def __init__(self, img):
		super().__init__(img)
		self.rect.x = random.randrange(80, s_width-30)
	...
	def update(self):
		self.rect.y += 1
		if self.rect.y > s_height:
			self.rect.x = random.randrange(80, s_width-10)
			self.rect.y = random.randrange(-4000, 0)
		self.shoot()

	def shoot(self):
		if self.rect.y in (0, 100, 300):
...
class Ufo(Enemy):
	...
	def shoot(self):
		if self.rect.x % 80 == 0:
			...
			ufobullet.rect.x = self.rect.x + 60
...
class Game:
	...
	def pause_text(self):
		...
		text = font.render('Pause', True, 'cyan')
		text_rect = text.get_rect(center=(s_width/2, s_height/2))
		screen.blit(text, text_rect)
	...
	def game_over_text(self):
		...
		text = font.render('GAME OVER', True, 'cyan')
		text_rect = text.get_rect(center=(s_width/2, s_height/2))
		screen.blit(text, text_rect)
	...
	def playerbullet_hits_enemy(self):
		...
		for i in hits:
			...
			if self.count_hit == 3:
				...
				i.rect.y = random.randrange(-2000, -100)
				self.count_hit = 0
				pygame.mixer.Sound.play(explosion_sound)
	...
	def run_game(self):
		...
		while True:
			...
			self.run_update()
			pygame.draw.rect(screen, 'navy', (0,0,s_width,30))
			self.create_lives()
			self.create_score()
